...
```{r}
#inhibitors_names <- read.csv('conformer_0_LuxR_above_15.csv')[, 1]

#leave.out <- sample(inhibitors_names, 5) # Done once - Should not run again

#leave.out 
#[1] "m_Br"    "m_Me"    "p_Cl"    "p_tfm"   "o_nitro"
leave.out <- c("m_Br", "m_Me", "p_Cl", "p_tfm", 'o_nitro')

all_models_3_3 <- lapply(list.files(pattern = 'LuxR_above_15.csv'), function(x) models.list.parallel(x, 3, 3, leave.out = leave.out, verbose = F))

names(all_models_3_3) <- list.files(pattern = 'LuxR_above_15.csv')
...
write.csv(top_models_4_4, 'LuxR_above_15_top_models_4_4_LO_5.csv')
```


```{r}
model.list <- "LuxR_above_15_top_models_4_4_LO_5.csv"
out.col = 'output'
what.model = 1
...
writeLines(output, "output_model_evaluation_LuxR.txt")
ggsave('LuxR_reg_4_4_LO_5.png', height = 11, width = 11)
```
