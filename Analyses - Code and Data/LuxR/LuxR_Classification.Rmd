...
```{r}
all_models <- parallel_train_models("LuxR_class", 
                                   one_ratio = 0.75, # smaller non-inhibtors class than in TraR and LuxR
                                   two_ratio = 0.65)

names(all_models) <- list.files(pattern = 'LuxR_class')
...
write.csv(top_models.tosave, 'LuxR_class_models.csv')

best_model <- retrain_best_model(top_models_file = 'LuxR_class_models.csv',
                   which_model = 2,
...
write.csv(best_model$validation_result, 'Class_validation_set_results_LuxR.csv')
